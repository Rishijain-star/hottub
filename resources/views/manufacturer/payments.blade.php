...
@section('title', __('panel.payments.title').' - '.__('panel.manufacturer_title'))
@section('content')
...
<div class="card mb-4" style="padding: 1.25rem;">
    <form method="GET" action="{{ route('manufacturer.payments') }}" class="panel-filter-form panel-filter-form--3">
        ...
        <div class="form-group mb-0 panel-filter-actions-col">
            ...
            <a href="{{ route('manufacturer.payments') }}" class="btn btn--ghost">{{ __('panel.common.clear') }}</a>
            </div>
        </div>
    </form>
</div>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td style="white-space:nowrap;">
                    <a href="{{ route('manufacturer.invoice', $inv->invoice_number) }}" class="text-teal fw-700">{{ __('panel.payments.view') }}</a>
                    <span class="text-muted" style="margin:0 6px;">|</span>
                    <a href="{{ route('manufacturer.invoice.download', $inv->invoice_number) }}" class="text-teal fw-700">{{ __('panel.payments.download') }}</a>
                </td>
            </tr>
            @empty
            <tr>
