            ...
            @forelse($recentRequests as $request)
                <tr>
                    ...
                    <td>
                        ...
                        @else
                            <a href="{{ route('manufacturer.service-requests') }}" class="btn btn--ghost btn--sm">View</a>
                        @endif
                    </td>
                </tr>
            @empty
...
<aside class="card mfr-overview-tasks" style="margin-top:0">
    ...
    <p class="text-sm text-muted" style="margin-bottom:0.75rem">Outstanding tasks for active and won leads</p>
    <ul id="mfrDashboardTasksList" style="list-style:none;padding:0;margin:0;display:flex;flex-direction:column;gap:0.75rem">
        ...
        <li class="dashboard-task-item" data-url="{{ route('manufacturer.leads.view', $task->lead_id) }}" style="padding:0.7rem 0.85rem;border:1px solid #e5e7eb;border-radius:8px;font-size:0.88rem;background:#fff;cursor:pointer;transition:box-shadow .15s ease,border-color .15s ease;">
            ...
            <div class="text-xs text-muted mt-1">Lead #{{ $task->lead_id }}@if($task->due_date) · Due {{ $task->due_date->format('d M Y') }}@else · No due date @endif</div>
        </li>
        @empty
    ...
    <div id="mfrDashboardTasksActions" class="d-flex justify-content-center mt-3" style="gap:0.5rem">
        <button type="button" class="btn btn--outline btn--sm" id="seeMoreMfrDashboardTasks">See More</button>
        <button type="button" class="btn btn--outline btn--sm" id="seeLessMfrDashboardTasks" style="display:none">See Less</button>
    </div>
    @else
