...
@section('title', __('panel.maintenance.title').' - '.__('panel.manufacturer_title'))
@section('content')
<style>
    .pkg-card {
        position: relative;
...
<div class="card" id="packageFormCard" style="display:none; margin-bottom: 20px;">
    ...
    <form id="packageForm" method="POST" action="{{ route('manufacturer.maintenance-packages.store') }}">
        @csrf
    ...
    <div class="pkg-card">
        ...
        <div class="pkg-header">
            ...
            <div style="display:flex; gap:8px;">
                ...
                <form method="POST" action="{{ route('manufacturer.maintenance-packages.destroy', $pkg) }}" onsubmit="event.preventDefault(); showConfirmationModal(this, '{{ __('panel.maintenance.delete_package_title') }}', '{{ __('panel.maintenance.delete_package_body') }}');">
                    @csrf @method('DELETE')
...
function showAddForm() {
    ...
    document.getElementById('packageForm').action = "{{ route('manufacturer.maintenance-packages.store') }}";
...
async function editPackage(id) {
    try {
        const res = await fetch(`/manufacturer/maintenance-packages/${id}/edit`);
        const data = await res.json();
        if (data.ok) {
            const pkg = data.package;
            document.getElementById('formTitle').textContent = @json(__('panel.maintenance.edit_package'));
            document.getElementById('packageForm').action = `/manufacturer/maintenance-packages/${id}`;
