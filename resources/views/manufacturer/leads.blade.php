...
@section('title', __('panel.leads.title').' - '.__('panel.manufacturer_title'))
@section('content')
...
<div class="card mb-4">
    <form method="GET" action="{{ route('manufacturer.leads') }}" class="panel-filter-form panel-filter-form--3">
        ...
        <div class="form-group mb-0 panel-filter-actions-col">
            ...
            <a href="{{ route('manufacturer.leads') }}" class="btn btn--ghost">{{ __('panel.common.clear') }}</a>
            </div>
        </div>
    </form>
</div>
    ...
    <table class="table">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>
                    <a href="{{ route('manufacturer.leads.view', $it->id) }}" class="btn btn--ghost btn--sm">{{ __('panel.common.view') }}</a>
                    <form action="{{ route('manufacturer.leads.private.destroy', $it->id) }}" method="POST" style="display:inline-block; margin-left: 6px;" onsubmit="event.preventDefault(); showConfirmationModal(this, '{{ __('panel.leads.delete_lead_title') }}', '{{ __('panel.leads.delete_lead_body') }}', '{{ __('panel.leads.delete_lead_confirm') }}');">
                        @csrf
                        @method('DELETE')
    ...
    <table class="table">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>
                    <a href="{{ route('manufacturer.leads.view', $it->id) }}" class="fw-700 text-dark" style="text-decoration:none">{{ __('panel.common.view') }}</a>
                </td>
            </tr>
            @empty
