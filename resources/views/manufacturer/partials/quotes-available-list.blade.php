    ...
    <table class="table">
        ...
        <tbody>
            ...
            <tr data-lead-id="{{ $it->id }}">
                ...
                <td><span class="text-sm">{{ trans_choice('panel.common.manufacturers_purchased', $cnt, ['count' => $cnt]) }}</span></td>
                <td>
                    @if($iBought)
                        <a class="btn btn--ghost btn--sm" href="{{ route('manufacturer.leads.view', $it) }}">{{ __('panel.common.view') }}</a>
                    @elseif($cnt >= 3)
                    ...
                    @else
                        ...
                        <form action="{{ route('manufacturer.leads.decline', $it->id) }}" method="POST" style="display:inline-block; margin-left: 6px;" onsubmit="event.preventDefault(); showConfirmationModal(this, @json(__('panel.quotes.decline_confirm_title')), @json(__('panel.quotes.decline_confirm_body')), @json(__('panel.quotes.decline_confirm_yes')));">
                            @csrf
