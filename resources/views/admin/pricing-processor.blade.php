...
<form method="POST" action="{{ route('admin.pricing-processor.save') }}">
    @csrf
    <div class="card">
        <div class="fw-800 mb-2" style="font-size:1.05rem;color:var(--gray-900)">Credit Purchase Method</div>
        <div class="grid grid--3">
            <label class="form-check" style="display:flex;align-items:center;gap:8px">
                <input type="radio" name="active_processor" value="manual" @checked(optional($settings)->active_processor==='manual' || !optional($settings)->active_processor)> Manual Management
            </label>
            <label class="form-check" style="display:flex;align-items:center;gap:8px">
