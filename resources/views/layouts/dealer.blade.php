@php
    $dealer = auth()->user();
    $unreadMessages = \App\Models\Message::where('receiver_id', $dealer->id)->whereNull('read_at')->count();
    $pendingServices = \App\Models\ServiceRequest::where('dealer_id', $dealer->id)->where('status', 'pending')->count();
    $pendingPackages = \App\Models\PackageRequest::where('dealer_id', $dealer->id)->where('status', 'pending')->count();
    $totalNotifications = $unreadMessages + $pendingServices + $pendingPackages;
@endphp
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    @if(isset($isAccountRestricted) && $isAccountRestricted)
        <div style="position:fixed; inset:0; background:rgba(15, 23, 42, 0.9); backdrop-filter:blur(8px); z-index:99999; display:flex; align-items:center; justify-content:center; padding:1.5rem;">
            <div class="card" style="max-width:550px; width:100%; padding:3.5rem 2rem; text-align:center; border:2px solid #ef4444; box-shadow:0 25px 50px -12px rgba(239, 68, 68, 0.25);">
                ...
                <p style="color:#4b5563; font-size:1.1rem; line-height:1.6; margin-bottom:2.5rem;">Your dealership account has been <strong>{{ $restrictionStatus }}</strong> by the administrator. All functionality has been locked. Please contact support to resolve this.</p>
                ...
                <div style="background:#f9fafb; padding:2rem; border-radius:12px; border:1px solid #e5e7eb; text-align:left;">
                    ...
                    @if($requestCount < 3)
                        <form action="{{ route('dealer.api.send_message', 1) }}" method="POST">
                            @csrf
