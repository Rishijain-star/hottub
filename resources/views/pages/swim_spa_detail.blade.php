@extends('layouts.app')
@section('title', ($item->brand ?? '').' '.($item->model ?? '').' | '.__('pages.detail.swim_spas_title_suffix'))
@section('meta_description', __('pages.detail.meta_description_short', ['brand' => $item->brand, 'model' => $item->model]))

@section('content')
<div class="container" style="margin-top:24px;margin-bottom:24px">
    <a href="{{ route('swim-spas') }}" style="display:inline-block;color:#0ea5a3;text-decoration:none;font-weight:700">{{ __('pages.detail.back_to_swim_spas') }}</a>
        ...
        <div class="product-detail-summary" style="background:#fff;border-radius:14px;box-shadow:0 10px 30px rgba(0,0,0,.06);padding:24px">
            ...
            <div style="display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px;margin-bottom:16px">
                <div style="display:flex;align-items:center;gap:10px;background:#f8fafb;border:1px solid #e5e7eb;border-radius:12px;padding:12px">
                    ...
                    <div><div style="font-size:12px;color:#6b7280">{{ __('pages.detail.seating') }}</div><div style="font-weight:700">{{ $item->seats ?? '—' }}</div></div>
                </div>
                <div style="display:flex;align-items:center;gap:10px;background:#f8fafb;border:1px solid #e5e7eb;border-radius:12px;padding:12px">
            ...
            <button onclick='window.__openEnquiryModal({ title: @json(__('pages.detail.quote_title', ['brand' => $item->brand, 'model' => $item->model])), type: "swim_spa", product_id: "{{ $item->id }}" })' class="ht-get-quote-btn" style="display:block; width:100%; text-align:center;">{{ __('pages.detail.get_free_quote') }}</button>
