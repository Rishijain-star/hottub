...
<div class="container" style="margin-top:24px;margin-bottom:24px">
    <a href="{{ route('hot-tubs') }}" style="display:inline-block;color:#0ea5a3;text-decoration:none;font-weight:700">← Back to Hot Tubs</a>
        ...
        <div style="background:#fff;border-radius:14px;box-shadow:0 10px 30px rgba(0,0,0,.06);padding:24px">
            ...
            <div style="display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px;margin-bottom:16px">
                <div style="display:flex;align-items:center;gap:10px;background:#f8fafb;border:1px solid #e5e7eb;border-radius:12px;padding:12px">
                    ...
                    <div><div style="font-size:12px;color:#6b7280">Seats</div><div style="font-weight:700">{{ $item->seats ?? '—' }}</div></div>
                </div>
                <div style="display:flex;align-items:center;gap:10px;background:#f8fafb;border:1px solid #e5e7eb;border-radius:12px;padding:12px">
            ...
            <a href="#quote" id="btnGetFreeQuote" class="ht-get-quote-btn" style="display:inline-block;text-align:center">Get Free Quote</a>
...
document.getElementById('btnGetFreeQuote')?.addEventListener('click', function(e){
    e.preventDefault();
    window.__openEnquiryModal({
        title: 'Request {{ $item->brand }} {{ $item->model }}',
        subtitle: 'Fill in your details and an approved dealer will contact you.',
        type: 'hot_tub',
        timeframe: null,
        product_id: '{{ $item->id }}'
    });
