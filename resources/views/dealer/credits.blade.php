...
@section('title', 'Credits – Dealer Panel')
@section('content')
<div class="panel-page-header">
    <div>
        <h1 class="panel-page-title">Credits</h1>
        <p class="panel-page-sub">Request and manage your account credits</p>
    </div>
    ...
    <script>
        ...
        document.querySelectorAll('.js-buy-plan').forEach(btn => {
            btn.addEventListener('click', async function() {
                ...
                try {
                    const res = await fetch('{{ route('dealer.credits.purchase') }}', {
                        method: 'POST',
                        headers: {
                    ...
                    if (data.id && stripe) {
                        ...
                        alert('Payment gateway is not configured. Please contact admin.');
                        this.disabled = false;
    ...
    <div class="card" style="padding: 0;">
        <div style="padding: 1.25rem; border-bottom: 1px solid var(--gray-200); display: flex; justify-content: space-between; align-items: center;">
            ...
            <form method="GET" action="{{ route('dealer.credits') }}" style="display: flex; gap: 0.5rem; align-items: center;">
                ...
                @if(request('status'))
                    <a href="{{ route('dealer.credits') }}" class="btn btn--ghost btn--sm">Clear</a>
                @endif
            </form>
        </div>
        <table class="table">
