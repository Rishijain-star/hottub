...
@section('title', __('panel.credits.title').' – '.__('panel.dealer_title'))
@section('content')
...
<div class="grid grid--2" style="gap: 1.5rem; align-items: start;">
        ...
        <div class="plans-grid" style="display:grid;grid-template-columns:repeat(auto-fill, minmax(200px, 1fr));gap:20px;margin-bottom:20px">
            @forelse($plans as $plan)
                <div class="plan-card" style="border:1px solid var(--gray-200);border-radius:12px;padding:20px;text-align:center;display:flex;flex-direction:column;gap:10px;transition:all 0.2s;background:white">
                    ...
                    <div class="text-xs text-muted mb-1">{{ __('panel.credits.credits_label') }}</div>
    ...
    <div class="card" style="padding: 0;">
        <div style="padding: 1.25rem; border-bottom: 1px solid var(--gray-200); display: flex; justify-content: space-between; align-items: center;">
            ...
            <form method="GET" action="{{ route('dealer.credits') }}" style="display: flex; gap: 0.5rem; align-items: center;">
                <select name="status" class="form-input" style="padding: 0.25rem 0.5rem; font-size: 0.85rem; width: auto;">
                    ...
                    <option value="approved" {{ request('status') === 'approved' ? 'selected' : '' }}>{{ __('panel.status.approved') }}</option>
                    <option value="rejected" {{ request('status') === 'rejected' ? 'selected' : '' }}>{{ __('panel.status.rejected') }}</option>
                </select>
                <button type="submit" class="btn btn--primary btn--sm">{{ __('panel.common.filter') }}</button>
                @if(request('status'))
                    <a href="{{ route('dealer.credits') }}" class="btn btn--ghost btn--sm">{{ __('panel.common.clear') }}</a>
                @endif
            </form>
        </div>
        <table class="table">
...
@php
    $creditsPanelI18n = [
        'redirecting' => __('panel.credits.redirecting'),
        'paymentNotConfigured' => __('panel.credits.payment_not_configured'),
        'failedInitPayment' => __('panel.credits.failed_init_payment'),
        'connectionError' => __('panel.credits.connection_error'),
    ];
@endphp
<script src="https://js.stripe.com/v3/"></script>
<script>
const panelI18n = @json($creditsPanelI18n);
const stripePublishableKey = @json($stripePublishableKey ?? '');
const purchaseUrl = @json(route('dealer.credits.purchase'));
...
document.querySelectorAll('.js-buy-plan').forEach(btn => {
    btn.addEventListener('click', async function() {
        ...
        this.disabled = true;
        this.innerText = panelI18n.redirecting;
        ...
        try {
            ...
            if (data.id && stripe) {
                ...
                alert(panelI18n.paymentNotConfigured);
                this.disabled = false;
                this.innerText = originalText;
            } else {
                alert(data.error || panelI18n.failedInitPayment);
                this.disabled = false;
                this.innerText = originalText;
            }
        } catch (e) {
            alert(panelI18n.connectionError);
            this.disabled = false;
            this.innerText = originalText;
        }
    });
