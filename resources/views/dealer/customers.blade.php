...
@section('title', 'My Customers – Dealer Panel')
@section('content')
    ...
    <form method="GET" action="{{ route('dealer.customers.index') }}" class="panel-filter-form panel-filter-form--2">
        ...
        <div class="form-group mb-0 panel-filter-actions-col">
            ...
            <div class="panel-filter-actions">
                ...
                <a href="{{ route('dealer.customers.index') }}" class="btn btn--ghost">Clear</a>
            </div>
        </div>
    </form>
</div>
            ...
            <tbody>
                @foreach($customers as $lead)
                    ...
                    <tr style="border-bottom:1px solid var(--gray-100);vertical-align:top;">
                        ...
                        <td style="padding:0.75rem 0.5rem;">
                            @if($chatUser)
                                <a href="{{ route('dealer.messages', ['with' => $chatUser->id]) }}" class="btn btn--primary btn--sm">Chat</a>
                            @else
...
<div class="modal-backdrop" id="cancelReasonModalDealer" role="dialog" aria-modal="true" aria-labelledby="cancelReasonModalDealerTitle">
    <div class="modal" style="width:min(520px,94vw)" onclick="event.stopPropagation()">
        <div class="modal-header">
            <div class="modal-title" id="cancelReasonModalDealerTitle">Cancellation Reason</div>
            <button type="button" class="modal-close" id="cancelReasonModalDealerClose" aria-label="Close">&times;</button>
        </div>
...
<script>
    (function () {
        function openCancelReasonDealer(customer, reason) {
            document.getElementById('cancelReasonModalDealerCustomer').textContent = customer ? ('Customer: ' + customer) : '';
            document.getElementById('cancelReasonModalDealerText').textContent = reason || 'No reason provided.';
